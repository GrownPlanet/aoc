    ...
    let result = map
        ...
        .map(|(y, v)| {
            v
                ...
                .enumerate()
                .map(|(x, &p)| if p == 0 { 
                    count_hiking_trails((x, y), &map).len()
                } else { 
                    0 
                })
                .sum::<usize>()
...
fn count_hiking_trails((x, y): (usize, usize), map: &[Vec<usize>]) -> Vec<(usize, usize)> {
    let cur_num = map[y][x];
    if cur_num == 9 {
        return vec![(x, y)];
    }
    ...
    let mut hiking_trails: Vec<(usize, usize)> = next_pos
        .iter()
        .flat_map(|&p| count_hiking_trails(p, map))
        .collect();
    hiking_trails.sort();
    hiking_trails.dedup();
    hiking_trails
